...
def flatten_dataset_to_tensor(dataset, size):
    dataset_size = find_dataset_size(dataset) if size is None else size
...
def context_batch(dataset, batch_size, seq_len, dataset_size=None):
    ...
    if dataset_size is None:
        print("Computing size of flattened dataset...")
    ...
    print("Context batching...")
    dataset = dataset.window(seq_len, seq_len).flat_map(lambda w: w.batch(seq_len, drop_remainder=True))
...
def write_to_tfrecord_shards(directory, dataset, batch_size, seq_len, shards):
    ...
    for i in range(shards):
        ...
        print(f"Loading shard {i+1}/{shards}...")
        flat_ds = flatten_dataset_to_tensor(ds, shard_size)
    ...
    print(f"Total time to convert data to tfrecords: {time.time()-start:.2f}s\n\n")
...
class DataManager:
    ...
    def get_default_configs():
        configs = {
            ...
            'vocab_size': 12000,
            'sp_model_prefix': 'wiki2_'+str(12)+'k',
            'lowercase': True,
            'shards': 1,
            'batch_size': 32,
            'seq_len': 32,
